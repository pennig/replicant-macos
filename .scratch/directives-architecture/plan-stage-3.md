...
- **Two numbers for one `DirectiveEngineTests` run are not a disagreement.** The console prints a test-function count; the event stream also emits one `testStarted` per `@Suite`. `1671 tests in 221 suites` and `1892` are the SAME run: `1671 + 221 = 1892`. **Quote the console figure (test count) and, separately, the suite count** — never sum them into one number, and never read a ticket citing `1892` as a bigger run than one citing `1671`.
...
| C13 | 5 | `RelayRun.acquire`'s fleet-evidence re-read is skipped whenever the reserve rail already vetoes, because the gate now sits after the rail rather than before it | `BrainDegradationTests` "the reserve rail vetoes the print and the brain idles instead of thrashing" |
