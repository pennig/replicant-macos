...
**Combined worst case** (now live, not moot, and now scoped to retryable verbs only — the four `nonRetryableKinds` above stall on attempt 1, no multiplier): `.commandFailed`'s `brainDisposition == .retry` means `Brain`'s existing bounded auto-retry (`Brain.retryBudget = 3`) applies with no brain code change. A mission hitting a persistent 503 on a RETRYABLE verb: `failedDispatchBudget` (3 in-step retries) exhausts → stall `.commandFailed` → the brain retries the stall up to `Brain.retryBudget` (3) times, each re-entering the step and re-arming the in-step counter (the same retry-re-arms-the-counter shape recorded in `brain-salvage-build.md`). **3 brain retries × 3 in-step dispatch retries = up to 12 dispatch attempts** before the brain's own budget exhausts and the stall escalates to an operator. On `print`/`collectResources`/`depositResources`/`dequeuePrint` the in-step multiplier is 1, not 3, so the worst case there is **3 brain retries × 1 = up to 3 dispatch attempts**.
...
Commits: `e40bbe1` (WIP checkpoint — vocabulary + retry counter, before the governor fix), `037852c` (`fix(governor): exclude .failed ops from the per-step de-dup`), `7c2dac3` (first resolved bookkeeping, superseded by this paragraph), `6241ddc` (`fix(engine): non-idempotent verbs get no in-step retry on .failed`), plus this final bookkeeping commit.
