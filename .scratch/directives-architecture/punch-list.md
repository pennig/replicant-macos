...
- [ ] **Two print sites remain outside `PrintJob`.** `EventRun.swift:297` (`printing`) and `RelayRun.swift:359` (`printing`) keep their own poll shape; Stage 3 unifies all five — tickets 36 (`EventRun`) and 37 (`RelayRun`). **Note the second line number is wrong:** `RelayRun`'s dispatch is in `acquire` at `RelayRun.swift:348`, not in `printing`.
