...
- [ ] **`StowOrAttach`'s `.noSubject` is unreachable at all six migrated sites.** None looks a device up by code: `EventRun.loading`/`staging` and `MineRun.attach`/`detach`/`adopt` all derive their subjects from `world.devices.values`, and `EventRun.recovering` takes them from `EventRun.convoy` (`EventRun.swift:110-122`), which resolves every hull out of the same dictionary. Each arm is written to match the site's own missing-row behaviour (`.refreshFleet(tag:thenStall:)` at five of six) but none can be proven by a failing test. Live the moment a caller builds `deviceCodes` from a persisted list rather than the live snapshot.
- [ ] **`StowOrAttach`'s `.more` arm is unreachable — the fifth instance of the pattern.** `next` has three return paths and none is `.more`, yet six mission sites must switch exhaustively. Same shape as `TravelTo` (nine sites), `ReturnHome`, `BotPhase.awaitRepair` and `PrintJob`. A `StepResult` split — a two-case result for sub-machines that never loop — would retire all of them at once; ticket 32 is the place.
- [ ] **Two safety guards in the migrated pair were provably uncovered before this ticket.** Deleting `MineRun.attach`'s `guard roster.count == Self.carriedTotal` (`MineRun.swift:324-326`) left the full 1766-test target green, and so did deleting `EventRun.recovering`'s busy-carrier wait (`EventRun.swift:738`). Both now have a regression test (`MineRunTests.swift:525`, `EventRunReturnTests.swift:35`), but two probes in one file pair found two holes — a systematic mutation sweep over the engine's guards is likely to find more.
- [ ] **`EventRun.recovering`'s busy-carrier wait sits inside the `.action` arm, not ahead of `job.next`.** Deliberate: shipped code checked "courier already aboard" before the busy guard, so hoisting the guard would answer `.wait` where the mission used to advance to `returning`. Because the guard was untested until this ticket, its ordering is still untested — no test distinguishes the two placements.
- [ ] **`BrainLoopTests.theTimerLoopItselfTicksOnSchedule` (`Tests/BrainLoopTests.swift:158`) flaked once** under a full-target run and passed on five others. Its own doc already records that the neighbouring cadence assertion was deleted for the same non-reproducibility. It waits on `clock.advance(by: .zero)` rather than a real signal.
