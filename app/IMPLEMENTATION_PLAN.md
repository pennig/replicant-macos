...
- [ ] Poll coordinator: request coalescing (dedupe in-flight by resource), per-type TTL, budget-aware deferral using Phase-0 snapshot, visibility gating.
- [ ] `TimelineView`-driven progress bars/ETA from `completes_at` (zero-read interpolation).
