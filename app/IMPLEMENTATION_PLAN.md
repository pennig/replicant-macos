> **⚠️ HISTORICAL DOCUMENT — restored 2026-07-21 with this errata header.**
> This plan (2026-06-25) was written against the custom Rust relay and was
> deleted once built; it is restored because eleven production file headers
> cite its section numbers (`§5.1`, `§5.3`, `§6`, `§10`, …) — those citations
> remain valid: **the section numbering below is unchanged.** Read it as the
> *rationale record* for the sync engine's shape, with this translation table
> for what the 2026-07-19 SSE migration (backend v2.3.0) replaced:
>
> | Plan says (dead) | Codebase has (live) |
> | --- | --- |
> | Custom Rust relay, `RELAY_CLIENT_TOKEN`, `/api/events` polling | First-party `GET /v1/events/stream` (SSE), ordinary session token (`EventStreamClient`) |
> | `RelayClient` / `UnifiedEvent` / fingerprint dedup | `EventStreamClient` + `EventPipeline` + `GameEventEnvelope`; Redis stream-id dedup (`BoundedEventIDSet`) |
> | `RelayRoute` registry | `EventRoute`/`EventMatcher` (GameServices) + `EventRouter` (GameSync); routes exported by owning modules (`MessagesIngestion`, `LocationsIngestion`, …) |
> | Per-replicant game-log backfill | Account-wide cursor-gated catch-up (`GET /v1/events`, 15-min window, 2000-event cap) |
> | `relay_*` event names (`device_travel_arrived`, `print_complete`, …) | Dotted taxonomy (`travel.arrived`, `print.completed`, `site.depleted`, …) |
> | `DependencyClients` module | `GameServices` (engine + domain clients) over `GameSession` (GameClient + Keychain) |
>
> The V3 review (`ARCHITECTURE_REVIEW.md`, 2026-07-20) re-verified the engine
> against this plan post-migration; its V3.10 punch list records what was
> tightened since. For the current how-it-fits-together map, read `app/README.md`.
