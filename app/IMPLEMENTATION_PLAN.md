...
- [x] Single deadline timer over open `Operation.completesAt` (skip nil-deadline enqueued ops; bounded backoff fallback for them). _`DeadlineScheduler` (GameSync actor): sleeps to the earliest open `active`+`completesAt` op (capped re-check for newly-inserted ops), and on the deadline takes one high-priority confirm-read + closes the op — unless a relay completion event already did (then it's not open → no read). Travel completion now lands either way: relay `device_cruise_arrived` (added to `Reconciler.completionEventTypes`) or this backstop. **Deferred:** the nil-deadline enqueued backoff fallback (print currently completes via `print_complete`); not yet a timed fallback._
...
- [ ] `DevicesFeature` module: devices list + detail (capacity ring, active-task card, parameterized/confirmable command grid per DESIGN_SPEC).
- [ ] Global "Operations/Activity" view observing `Operation` across the fleet.
- [ ] Minimal Bobnet view over `BobnetMessage` (best-effort, locally persisted).
- **Ship criterion:** the three-panel device experience is real and live; the action layer has a home.
