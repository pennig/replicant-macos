...
**Stuck-sheet recovery (2026-07-23):** even with `.sheet(item:)`, AppKit can refuse the presentation outright — `-[NSWindow _beginWindowBlockingModalSessionForSheet:]` fails while another window-blocking session (completion popover, alert, or a prior sheet mid-teardown) is still active. The item state then stays non-nil with no sheet on screen and NO recovery path: dismiss actions only fire from a visible sheet, and later presents swap the item's value without passing through nil, so SwiftUI never reconciles — the sheet looks permanently broken. Fix in `DevicesFeature.clearStalePresentations`: every present flow (`travelConfirmed` / `printConfirmed` / `cargoLoadTapped` / `directiveComposeTapped`) first clears ALL presentation state + cancels its effects (a grid tap is impossible under a real modal, so anything non-nil there is provably stale), logs a warning naming what was stuck, then does the endEditing + runloop-yield dance before presenting fresh. The other travel entry points (NewStarMapFeature, LocationsFeature) do NOT have this guard yet — replicate it there if a stuck travel sheet is ever reported from the map or a location dossier.
