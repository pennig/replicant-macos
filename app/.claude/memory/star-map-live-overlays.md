...
- **Ships**: from `Device`s with a travel `derivedActivity` — origin/destination systems from `travelSnapshot`, trip window from `startedAt`/`completesAt`. `Ship` now uses media-time `departedMedia`/`arrivesMedia` (clamped, no loop); renderer converts the wall-clock `ShipRoute` dates once at build. Was two hard-coded demo ships. **Since 2026-07-23 (`0cbae94`) the tappable device icon over each pip is GPU-drawn in the renderer itself** (`ShipIconTextureCache` + `encodeShipIcons`; clicks/hover via `StarMTKView`→`pickShip`) — there is no SwiftUI ship overlay any more; `ShipRoute` carries `deviceType` for the glyph. See [[starmap-main-thread-load]].
