...
5. **Consolidate the six composite views into `Controls.swift`** + add display/micro/tile/hairline tokens (V2.4). — **partly done (2026-07-03).** Added tokens (`Font.rcDisplay`/`rcMicro`, `Hairline`, `TileSize`) and controls `RCErrorBanner` (adopted ×5, byte-identical), `RCGlyphTile` (adopted in Devices/PrintQueue/Blueprints; Replicants left — its tile carries `isOwn` selection styling the generic control doesn't cover), and `RCSectionHeader`/`RCReadoutCard` (hoisted from LocationDetailView's private cards + adopted there). **Remaining:** `RCPill`, `RCMeterBar`, `RCDetailRow`, and broad section-header/font-token adoption — deferred as higher per-site variation warranting visual verification.
