...
5. ~~**Audit trail**~~ — **shipped as `DirectiveLogEntry`**, which carries precisely the pair the
   ...
   you are watching. **One residual gap: `.opCompleted` entries are still unwritten** (the kind,
   its glyph and its test exist; no writer does). A long travel therefore reads as a quiet gap in
   the timeline until the next step starts, which the pane's "Now" readout covers. Closing it is
   engine work — the executor would have to notice a dispatched op closing.
