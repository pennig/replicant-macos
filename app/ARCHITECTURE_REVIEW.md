...
Blocking gaps, in order — **ALL FIVE CLOSED as of 2026-07-28**; the automations feature
(Directives, Stages 1–4 + stall resolution + step timeline) answered 3–5 from inside itself,
exactly as its design predicted. Verified in code 2026-07-28, not merely assumed from the plan:

1. ~~**Replay immunity (hard blocker)**~~ — closed by P0 items 1/5 (sequenced catch-up, monotonic
   cursor, generation stamp) *and*, for the engine specifically, obsoleted by the clock-driven
   design in 4 below.
2. ~~**Hot-path isolation**~~ — closed by P1 item 8 (`DomainFreshness`: routes only `invalidate`,
   nothing slow runs on `EventRouter.dispatch`). The engine additionally never registers a
   command-issuing route at all.
3. ~~**Budget-aware dispatch**~~ — **shipped as `CommandGovernor`** (GameServices,
   `@Dependency(\.commandGovernor)`; Directives Stage 3). It answers both halves of the finding:
   it reads `gameClient.budget(.actions)` and defers below a reserve floor of 6 (proportional to
   `PollCoordinator`'s 12 of 120 reads), and it holds a **per-device in-flight claim** released on
   *every* path including rejection. A refusal is `.deferred`, which writes nothing and is not a
   failure — the step is late, never lost.
4. ~~**Loop protection**~~ — **obviated rather than implemented, and the substitute is stronger.**
   V3.9 prescribed suppressing triggers for N seconds after own-dispatch. The engine instead
   evaluates on a **5s `continuousClock` tick over reconciled SQLite state and never subscribes to
   events at all**, so its own command echo has no trigger path to re-enter. There is no
   suppression window to tune, and no window-sized hole. Missions wait on op *identity*, not on
   event arrival. (Do not "optimize" this into an event-kicked executor — the immunity is the
   point.)
5. ~~**Audit trail**~~ — **shipped as `DirectiveLogEntry`**, which carries precisely the pair the
   finding asked for: `eventID` (the SSE event that produced the entry) → `operationID` (the op it
   created or closed), plus `directiveID`/`deviceCode` so one table serves both custom missions and
   built-in AMI directives. It is browsable in the Directives detail pane's step timeline rather
   than the Event Log window — a better home than the one suggested, since it is scoped to the run
   you are watching. ~~One residual gap: `.opCompleted` entries are still unwritten~~ — **closed
   2026-07-28.** `DirectiveExecutor.recordCompletedOps` runs each tick ahead of the machine and
   appends an entry for every dispatched op that has since reached a terminal state, so a long
   travel no longer reads as an unexplained quiet stretch. It is **audit-only by construction** —
   it writes log rows and touches no directive status, step or target, which is what makes it safe
   on a 5s loop; delete it and execution is byte-identical. It fires for any terminal status (a
   silent *failed* op was as much of a gap as a silent completed one) and is idempotent through the
   log itself.

**Consequence for further automations work: nothing here blocks it.** Stage 5 (Relay Run + the
FTL-mesh incremental add) is unblocked and independent of the residual above.
...
10. ~~Payload application + S9~~ — done with a live-traffic correction: **per-leg arrival events don't exist post-migration** (docs catalogue + full retained-stream walk, 2026-07-21); the envelope's first-class `location` is the device's post-event position (null in transit/stowed). `Reconciler.applyEventFields` folds it in for every device event, guarded at-or-newer by event time with the stamp clamped to the local clock. **S9 remains open** — no `print.*` event exists in retained traffic and the catalogue documents only `print.started`; the route now logs a loud `⚠️ print.completed WITHOUT new_device_code` notice (with actual payload keys) the moment a real completion disproves the assumed key. **S9 CLOSED 2026-07-28 — the key is confirmed.** Four real `print.completed` events accumulated in the local `eventLogs` ledger (2026-07-26/27) and every one carries `new_device_code`, across three device types (`transport_drone`, `ftl_beacon`, `ftl_relay`) and both print modes (`autofactory`, `vessel`), beside `device_type` and `print_mode`. This is server evidence rather than an echo of our own assumption: the ledger stores the wire payload **verbatim**, and the snake_case keys are the proof it was never normalized by our decoder. Note *how* it became checkable — the server's retained event window no longer reaches those prints (a filtered `GET events` walk returns nothing), so **the ledger is what made S9 answerable at all**, which is a concrete argument for P0 item 6's decision to exempt `EventLog` from the logout wipe. **The loud notice has been removed** (2026-07-28): it existed to discover an unknown key, and once the key is known it was the only handler in the codebase carrying bespoke code for a hypothetical server-side rename. No other event handler defends against that, a rename is unlikely, and the Event Log window shows the real payload whenever the question comes up again — so the special case earned its deletion rather than a re-scoping.
...
**P3 — the shining-example tranche:** — **all four done 2026-07-21** (commits `a71435e`…`efd7b94`, each subagent-reviewed pre-commit under the LSP comprehension protocol).
16. ~~`app/README.md` map + IMPLEMENTATION_PLAN restoration + relay banner + doc fixes (D1/D2/D4)~~ — done (`a71435e`): README front-door map (module tiers + both lifecycles + read-first path, every claim code-verified); IMPLEMENTATION_PLAN restored byte-identical under an SSE-errata translation table (§ numbering intact — the 11 citations resolve again; BobnetMessage's §4.5 mis-cite corrected); relay/README DECOMMISSIONED banner; StarMap README on SPM reality; IN_SYSTEM plan header admits all five phases landed 2026-07-14; empty `API/Sources/Relay/` removed.
17. ~~"relay" → "event stream" comment sweep (D3)~~ — done (`0c8227a`): 44 comment-only rewrites incl. stale event-name modernization (`scan_complete`→`scan.completed`, `print_complete`→`print.completed`); game-domain relay and explicit historical contrasts untouched; verified zero executable-line changes.
18. ~~`IconSize` tokens + sweep; RCErrorBanner; RCSectionHeader; logging (D5)~~ — done (`d6337af`): IconSize s/m/l/xl/hero/display + rcMicroMono + rcFieldLabel minted from the actual clusters; 60 icon literals, 12 text-token swaps, 14 spacings, 23 hairlines snapped; RCErrorBanner's two clones replaced; **RCSectionHeader adopted (31 sites), not deleted** — stat lockups/field labels/pills deliberately left as non-headers; one subsystem (`name.pennig.replicould`), category = module. The Login wordmark survives, annotated. ~~LoggingMiddleware's logger deferred (user edit in flight)~~ — **done 2026-07-28**: it was the last holdout, defaulting to `name.pennig.replicould.api` / `http`, which split HTTP logging out of the app's stream so `log stream --subsystem name.pennig.replicould` silently missed every request. Now `name.pennig.replicould` / `API`, matching `RateLimitMiddleware` so both halves of one request share a category. Both call sites take the defaults, so changing them was sufficient.
19. ~~Presentation dialect + PrintQueue sheet + loud testValues + IssueReporting (T1/T3/T4/T5/D6)~~ — done (`efd7b94`): last `.sheet(isPresented:)` preview sheet converted; dialect blessed in CLAUDE.md **and** its cancellation clause made true (print previews gained CancelIDs; all preview dismissals cancel — a dismissed preview's late response could stamp the next sheet); four quiet testValues → `unimplemented` (445 tests unaffected; CommandClient gains a previewValue); Reconciler's writes + the schema bootstrap report failures via IssueReporting instead of `try?`-vanishing. T6's smaller items remain open by choice (documented exceptions, Tagged types, XCTest stragglers — "adopt opportunistically or never").

**T6 closed out 2026-07-28.** Two items had already been resolved in passing by later work and only needed confirming; the rest were done or decided:
- **Done now.** The NewStarMap view-local `@FetchAll` block carries the written-down exception it was missing (why the renderer's read-only inputs are not reducer state, and what *is* still reducer state). `BlueprintDetailView`'s duplicate `Blueprint` query is gone — the feature's state already observed that table, so the view-local copy was a second observation that could render a different row for a frame. `PreferencesFeature`'s `BindingReducer` + `BindableAction` are deleted: the picker binds the `@Shared` projection directly, so no binding action was ever sent, and carrying the path implied a store write that did not work. `CommandGrid`'s policy is extracted to **`CommandAvailability`** (a SwiftUI-free namespace per the view-statics trap) — the candidate builders and every availability gate, now covered by `CommandAvailabilityTests`, which is the point of the move.
- **`expectNoDifference`/`customDump` adopted.** `swift-custom-dump` is now a declared package dependency (it was already in the graph transitively via TCA) and the new suite uses `expectNoDifference`, so a failed collection comparison prints a structural diff. Adopt it in new tests where values are compared; there is no value in retrofitting passing assertions.
- **Already true, verified not assumed.** Zero XCTest files remain anywhere in the package (the two stragglers are gone). Zero raw `Task.sleep` debounces remain in the composition root or the service tier — `DomainFreshness` subsumed them exactly as this finding predicted it would.
- **Tagged types + `CommandParams`: DECIDED — NO, deliberately.** The finding asked for a decision rather than an outcome, so: the codes stay raw `String`. They are wire values that cross the generated OpenAPI boundary as `String` in both directions and land in SQLite through `QueryBindable`; Tagged would need conformances at both boundaries for every code type and would buy safety only in the thin middle, at a cost paid by every module. The confusion it would prevent has exactly one live instance — `DirectiveLogEntry`'s `directiveID`/`deviceCode` pair — and that is already guarded by routing both through a single `request(for:)`. `CommandParams` (now twelve optionals, up from nine) stays a flat bag for a related reason: the sum type the finding implicitly wants **already exists upstream** as `DeviceCommand`, whose associated values are mapped *into* `CommandParams` by `DeviceCommand.params(_:)`. The bag is a serialization boundary that mirrors one JSON body, not the domain model — which is the correct place for a flat optional struct. Revisit only if a command family appears whose params cannot be expressed this way.
