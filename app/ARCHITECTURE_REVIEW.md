...
8. Debounce FTL-mesh + message routes; move expensive applies off the router's critical path (B2/B3).
9. **Build the staleness model (V3.5)** — `StalenessTracker` + mark-mostly device route + drain loop + `refreshIfStale` + `DomainFreshness`. Converts B5/B6/B7/B9 and the visibility gap into one design.
10. Verify `print.completed`'s payload key via the Event Log window (S9); then start guarded payload application with per-leg arrivals (V3.5 table row 2).

**P2 — architecture hygiene:**
11. `UniverseModels` client split; `GameDatabase` below services (M1).
12. `CommandClient` family decomposition + `GameSession` extraction (M2).
13. Export `EventRoute`s from owning modules; shrink the composition root (M3 — do before Automations).
14. TCA → swift-dependencies in the four non-feature manifests (M4); M5–M7 cleanups.
15. GameModels test target (schema-mapping fixtures) + SSE-parser/catch-up tests.
