...
public struct MineRun: MissionStepMachine {
    ...
    /// Fly the emptied carrier back to the hub, where the next install and the
    /// print run's carrier slot both look for it. The destination is the hub
    /// LOCATION re-derived, never `originDesignation` — see `RelayRun.returnHome`.
    private func returnHome(
        _ directive: Directive, _ carrier: Device, _ world: WorldSnapshot
    ) -> MissionAction {
        // Nowhere to return to is done, not a stall: the mine is installed.
        guard let hub = RelayRun.hubLocation(in: world) else {
            logger.notice("mine run \(directive.id, privacy: .public): no hub to return to — leaving the carrier where it stands")
            return .done
        }
