...
@Suite struct YieldChartMathTests {
    ...
    @Test func pointsFollowDisplayOrderAndDropZeros() {
        ...
        let points = YieldChartMath.points(byDay: [YieldSummary.DayTotal(day: day, perType: cost)])
    ...
    @Test func labelsExactlyOneSegmentPerDay() {
        ...
        let points = YieldChartMath.points(byDay: [
            YieldSummary.DayTotal(day: dayA, perType: ResourceCost(structural: 100, rares: 10)),
            YieldSummary.DayTotal(day: dayB, perType: ResourceCost(structural: 5, rares: 200)),
        ])
    ...
    @Test func aTieBreaksTowardTheEarlierDisplayOrderSlot() {
        let day = Date(timeIntervalSince1970: 0)
        let points = YieldChartMath.points(byDay: [
            YieldSummary.DayTotal(day: day, perType: ResourceCost(structural: 50, conductive: 50)),
        ])
        let labelled = YieldChartMath.labelledIDs(points)
    ...
    @Test func aDayWithNoUnitsLabelsNothing() {
        let day = Date(timeIntervalSince1970: 0)
        let points = YieldChartMath.points(byDay: [YieldSummary.DayTotal(day: day, perType: ResourceCost())])
    ...
    @Test func tenSourcesKeepEightRowsAndTallyTheRest() {
        ...
        let sources = units.enumerated().map { YieldSummary.SourceTotal(designation: "SRC-\($0.offset + 1)", units: $0.element) }
    ...
    @Test func nineSourcesReturnVerbatimUnfolded() {
        let sources = (1...9).map { YieldSummary.SourceTotal(designation: "SRC-\($0)", units: $0 * 7 % 20) }
        let folded = YieldChartMath.foldedSources(sources)
    ...
    @Test func slicesAtOrAboveTenPercentAreLabelled() {
        let rows = [
            YieldSummary.ResourceTotal(key: "structural", units: 70),
            YieldSummary.ResourceTotal(key: "conductive", units: 15),
            YieldSummary.ResourceTotal(key: "silicates", units: 10),
            YieldSummary.ResourceTotal(key: "carbon", units: 5),
        ]
    ...
    @Test func aZeroTotalLabelsNoResourceKeys() {
        ...
        #expect(YieldChartMath.labelledResourceKeys([YieldSummary.ResourceTotal(key: "carbon", units: 0)]).isEmpty)
    }
}
