# Replicould — Design Spec

A reference for building the **Replicould** macOS app (a control surface for a
...
developer. Visual source of truth is the SwiftUI Previews in the swift files. 
Colors are codified in `Colors.xcassets` and `DesignSystem.swift`.
...
Use the named colors from `Colors.xcassets` (see `Color.rc*` in
`DesignSystem.swift`). Never hard‑code hex in views.
...
| `Separator` / `SeparatorStrong` / `SeparatorSoft` | hairlines (alpha) | white 9% / 16% / 5.5% | ink 12% / 22% / 7% |
| `TextPrimary/Secondary/Tertiary` | text ramp | `#E9EEF7` / `#9AA6BC` / `#697488` | `#1B2230` / `#5A6478` / `#929BAD` |
| `AccentPrimary` | selection, primary action | `#FFB23E` | `#D19317` |
| `AccentOnColor` | text on accent fills | `#2A1A05` | `#2A1A05` |
| `AccentMuted` / `AccentBorder` | selection bg / hairline | amber 12% / 34% | amber 13% / 45% |
| `NPCAccent` | NPC indicator | `#B58BFF` | `#7B4FD6` |
| `Danger` / `DangerMuted` | destructive | `#DE544A` | `#BB463C` |
...
### 3.5 Controls (`Controls.swift`)

**Buttons** — one `ButtonStyle` per variant: `RCButtonStyle(.primary | .secondary | .destructive | .destructiveProminent | .text)`. Pass `fullWidth: true` for form CTAs. Visual reference: `RCControlsGalleryView` in `Controls.swift`.

| Variant | Fill | Text | Use for |
|---|---|---|---|
| `.primary` | `AccentPrimary` (+ subtle top sheen, accent glow) | `AccentOnColor` | the main action (Deploy, Log in) |
| `.secondary` | transparent fill + `SeparatorStrong` hairline, faint hover fill | `TextSecondary`→`TextPrimary` on hover | Cancel / secondary actions |
| `.destructive` | transparent → faint `Danger` tint on hover, `Danger` hairline | `Danger` | the affordance that **opens** a confirm (Deactivate, Log out) |
| `.destructiveProminent` | solid `Danger` (+ sheen + danger glow) | white | the confirm that **fires** it |
| `.text` | none, accent underline on hover | `AccentPrimary` | inline / link-like |

Press/hover/disabled are handled inside each style (an inner view reads hover + `isEnabled` + `configuration.isPressed`). Commands stay parameterized — a `.destructive` trigger reveals the confirm panel; only its `.destructiveProminent` button executes.

**Text fields** — `RCField("Label", text:, placeholder:, hint:, mono:, secure:)` renders the uppercase caption, optional inline hint, focus ring (`AccentBorder`), and secure-with-reveal. To style a bare `TextField`/`SecureField` yourself, use `.rcField(focused:)` (pass a `@FocusState` bool — a `TextFieldStyle` can't see focus, so the ring lives in a modifier). Background `SurfaceRaised`, radius `Radius.control`.
