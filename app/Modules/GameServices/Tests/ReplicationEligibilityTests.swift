...
struct ReplicationEligibilityTests {
    ...
    /// Matrix at SOL-3 that can replicate, plus an empty matrix stowed in a
    /// cradle-equipped vessel at the same location → fully eligible.
    private func eligibleFleet() -> [Device] {
        [
            device(code: "MATRIX1", type: "replicant_matrix", location: "SOL-3", commands: ["replicate"]),
    ...
    func fullyEligible() {
        let result = ReplicationEligibility.resolve(hostDeviceCode: "MATRIX1", devices: eligibleFleet())
    ...
    func noEmptyMatrix() {
        ...
        let result = ReplicationEligibility.resolve(hostDeviceCode: "MATRIX1", devices: fleet)
        #expect(!result.canReplicate)
    ...
    func matrixCannotReplicate() {
        ...
        fleet = fleet.map { dev in
            ...
            return device(code: "MATRIX1", type: "replicant_matrix", location: "SOL-3", commands: [])
        }
        let result = ReplicationEligibility.resolve(hostDeviceCode: "MATRIX1", devices: fleet)
        #expect(!result.canReplicate)
