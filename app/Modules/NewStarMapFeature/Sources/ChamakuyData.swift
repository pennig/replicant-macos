...
//  NewStarMapFeature
//
//  Seed orrery for Chamakuy, ported from the SceneKit `StarMapFeature`. Scene
//  units are prototype pixels ÷ 10.
//
//  Chamakuy is the only modeled system; drilling into another scanned system
//  reuses this orrery relabeled with that star's identity. `model(for:)` does
//  that relabeling (and overwrites the star's position with the selected one, so
//  the seed position below is just a placeholder).
//
...
enum ChamakuyData {
    ...
    static func model(for system: GalaxySystem) -> SystemModel {
        var model = chamakuy
    ...
    static let chamakuy = SystemModel(
        star: StarDetail(
            designation: "CHK",
            name: "Chamakuy",
            spectralType: "K2 V",
