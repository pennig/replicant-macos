...
// would sit visibly frozen. (The moon swarm used to share this pipeline via a sibling
// `orrery_swarm_vertex` minus this rotation; swarm members are now real sphere
// impostors drawn through `orrery_body_vertex` like every other body, so that function
// is gone.)
