...
**LSP root is `Modules/`** (where `Package.swift` lives) — nearly all code is in that SPM package, which SourceKit-LSP resolves with no build-server config. Root the language server there, not at the repo top level. The thin app-target shell (`macOS/AppFeature.swift`, `MainFeature.swift`, `ReplicantApp.swift`) lives in the `.xcodeproj` and is *not* covered without an `xcode-build-server`-generated `buildServer.json`; grep is acceptable for that sliver. Build the package once so the index store is populated (a stale index yields stale symbol answers). The LSP tools load at Claude Code startup, so a session must be (re)launched with the Swift LSP plugin active to have them. **0 references is usually the index, not a config problem — but "wait and re-query" only helps for pre-existing code.** Indexing is lazy (driven by which files get opened, and it stops once those are covered), and each server imports the index store into a per-process database *once at startup*, so **a running session never picks up symbols added during that session — only a restart does.** In a fresh worktree the index starts empty, so same-session and new-file symbols will simply not resolve; that is expected, not a fault to debug. Diagnostics on code that demonstrably compiles (`No such module …`, `Cannot find X in scope`, `Type Y has no member 'where'`) are cold-index noise. Full detail, including two dead-end "fixes" not to retry, is in `.claude/memory/sourcekit-lsp-warmup.md`.
