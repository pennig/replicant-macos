...
- **Running `swift test`:** read results from Swift Testing's JSON event stream, never by parsing console text (text scraping is non-deterministic — a grep for "fail" false-positives on test method names). Use the **`swift-test-event-stream`** skill for the invocation and `jq` recipes. Pin `--event-stream-version 6.4`; plain `--filter` is correct on Swift 6.4 (the multi-target truncation trap is fixed, so `--test-product` is no longer needed), but a run now emits one `runStarted`/`runEnded` pair **per test product**, so gate completeness on that count rather than on a single `runEnded`.
