...
**In a new worktree, run `Modules/scripts/link-index-store.sh` after your first build.** SwiftPM advertises its index store at `.build/index-store` while the swiftbuild engine writes to `.build/out`; the script symlinks them, and without it every reference query returns zero. Re-run after anything that wipes `.build`.
