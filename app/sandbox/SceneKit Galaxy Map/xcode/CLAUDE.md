...
- **`ReplicantControls.swift`** — buttons (`RCButtonStyle(.primary/.secondary/.destructive/.destructiveProminent/.text)`), text fields (`RCField`, `.rcField(focused:)`), segmented control (`RCSegmentedControl`), and dropdowns (`RCValueSelect` for parameterized-command value pickers; `RCEntitySwitcher` for the ACTIVE REPLICANT picker — built on native `Menu` for the system popover).
