...
struct SidebarView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            replicantHeader
            Divider().overlay(.rcSeparatorSoft)
    ...
    private var replicantHeader: some View {
        VStack(alignment: .leading, spacing: 9) {
            ...
            Button { showSwitcher.toggle() } label: {
                HStack(spacing: 11) {
                    Image(systemName: active.host.sfSymbol)
                        .font(.system(size: 16)).foregroundStyle(.rcAccent)
                        .frame(width: 34, height: 34)
                        .background(.rcAccentMuted, in: RoundedRectangle(cornerRadius: 9))
                        .overlay(RoundedRectangle(cornerRadius: 9).strokeBorder(.rcAccentBorder, lineWidth: 0.5))
                    VStack(alignment: .leading, spacing: 2) {
                        Text(active.name).font(.system(size: 15.5, weight: .bold)).foregroundStyle(.rcTextPrimary)
                        HStack(spacing: 5) {
                            Text(active.host.label).font(.rcCaption).foregroundStyle(.rcTextSecondary)
                            if active.isNPC {
                                Text("·").foregroundStyle(.rcTextTertiary)
                                Image(systemName: SidebarSymbol.npc).font(.system(size: 10)).foregroundStyle(.rcNPC)
                            }
                        }
                    }
                    Spacer(minLength: 0)
                    Image(systemName: "chevron.down").font(.system(size: 11, weight: .semibold)).foregroundStyle(.rcTextSecondary)
                }
                .padding(.horizontal, 11).padding(.vertical, 9)
                .background(.rcSurfaceRaised, in: RoundedRectangle(cornerRadius: 11))
                .overlay(RoundedRectangle(cornerRadius: 11).strokeBorder(.rcSeparator, lineWidth: 0.5))
            }
            .buttonStyle(.plain)
            ...
            HStack(spacing: 8) {
                HStack(spacing: 0) {
                    Text(active.xp.formatted()).font(.rcMonoSmall).foregroundStyle(.rcTextPrimary)
                    Text(" XP · ").font(.rcMonoSmall).foregroundStyle(.rcTextTertiary)
                    Text("\(active.deviceCount)").font(.rcMonoSmall).foregroundStyle(.rcTextPrimary)
                    Text(" dev").font(.rcMonoSmall).foregroundStyle(.rcTextTertiary)
                }
                Spacer(minLength: 0)
                Button { } label: {
                    HStack(spacing: 3) { Text("Show in Replicants"); Image(systemName: "arrow.up.right") }
                        .font(.system(size: 10.5, weight: .semibold)).foregroundStyle(.rcAccent)
                }.buttonStyle(.plain)
            }

            TextField("Plan", text: $replicants[activeIndex].plan, axis: .vertical)
                .textFieldStyle(.plain).font(.rcBody).foregroundStyle(.rcTextSecondary)
        ...
        VStack(alignment: .leading, spacing: 2) {
            ForEach(navGroups, id: \.0) { title, items in
                Text(title).font(.system(size: 10, weight: .bold)).kerning(1).textCase(.uppercase)
                    .foregroundStyle(.rcTextTertiary).padding(.horizontal, 10).padding(.top, 14).padding(.bottom, 6)
        ...
        VStack(alignment: .leading, spacing: 6) {
            if replicant.statusRaw == "travelling", let to = replicant.travelTo, let rem = replicant.travelRemaining, let p = replicant.travelProgress {
                HStack(spacing: 7) {
                    ...
                    Text("Cruise → \(to)").font(.rcCaption).foregroundStyle(.rcTextPrimary)
                    Spacer(minLength: 0)
                    Text(rem).font(.rcMonoSmall).foregroundStyle(.rcTextTertiary)
                }
                ProgressView(value: p).tint(tone.color)
            } else {
                HStack(spacing: 7) {
                    Circle().fill(tone.color).frame(width: 6, height: 6)
                    Text("Idle").font(.rcCaption).foregroundStyle(.rcTextSecondary)
                    Spacer(minLength: 0)
                    Text(replicant.location).font(.rcMonoSmall).foregroundStyle(.rcTextTertiary)
                }
            }
        }
        .padding(.horizontal, 11).padding(.vertical, 9)
        .background(.rcSurfaceRaised, in: RoundedRectangle(cornerRadius: 10))
        .overlay(RoundedRectangle(cornerRadius: 10).strokeBorder(.rcSeparator, lineWidth: 0.5))
    }
}
...
struct SwitcherList: View {
    ...
    var body: some View {
                ...
                Button { activeIndex = idx; dismiss() } label: {
                    HStack(spacing: 10) {
                        Image(systemName: r.host.sfSymbol).font(.system(size: 15))
                            ...
                            .background(.rcSurfaceRaisedStrong, in: RoundedRectangle(cornerRadius: 8))
                        VStack(alignment: .leading, spacing: 1) {
                            HStack(spacing: 5) {
                                Text(r.name).font(.rcBodyEmph).foregroundStyle(.rcTextPrimary)
                                if r.isNPC { Image(systemName: SidebarSymbol.npc).font(.system(size: 9)).foregroundStyle(.rcNPC) }
                            }
                            Text("\(r.host.label) · \(r.hostName)").font(.system(size: 10.5)).foregroundStyle(.rcTextTertiary)
                        }
                        Spacer(minLength: 20)
                        Text(r.statusRaw == "travelling" ? "Transit" : "Idle")
                            .font(.system(size: 10.5)).foregroundStyle(.rcTextSecondary)
                    }
                    .padding(.horizontal, 10).padding(.vertical, 8)
                    .background(idx == activeIndex ? AnyShapeStyle(.rcAccentMuted) : AnyShapeStyle(.clear),
                                in: RoundedRectangle(cornerRadius: 8))
                }.buttonStyle(.plain)
            }
            Divider().overlay(.rcSeparatorSoft).padding(.vertical, 4)
            Button { } label: {
        ...
        .padding(6).frame(width: 280).background(.rcSurfaceRaised)
    }
}
...
struct NavRowView: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .background(selected ? AnyShapeStyle(.rcAccentMuted) : AnyShapeStyle(.clear), in: RoundedRectangle(cornerRadius: 8))
            .overlay(alignment: .leading) {
                if selected { Capsule().fill(.rcAccent).frame(width: 3, height: 18).offset(x: -8) }
            }
        }.buttonStyle(.plain)
    }
    ...
    @ViewBuilder private var badge: some View {
        if entry.soon {
            Text("soon").font(.system(size: 9, weight: .bold)).kerning(0.4).textCase(.uppercase).foregroundStyle(.rcTextTertiary)
        } else if entry.live {
            HStack(spacing: 5) {
                Circle().fill(.rcStatusReady).frame(width: 6, height: 6).shadow(color: .rcStatusReady.opacity(0.7), radius: 3)
                Text("live").font(.system(size: 10, weight: .semibold)).foregroundStyle(.rcStatusReady)
            }
        } else if let b = entry.badge {
            Text("\(b)").font(.rcMonoSmall).foregroundStyle(.rcAccentOnColor)
                .padding(.horizontal, 7).padding(.vertical, 1)
                .background(.rcAccent, in: Capsule())
...
struct AccountChip: View {
    ...
    var body: some View {
        Button { } label: {
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 6) {
                    Image(systemName: SidebarSymbol.account).font(.system(size: 11)).foregroundStyle(.rcTextTertiary)
                    Text("Logged in").font(.system(size: 9, weight: .bold)).kerning(1).textCase(.uppercase).foregroundStyle(.rcTextTertiary)
                }
                HStack(spacing: 10) {
                    VStack(alignment: .leading, spacing: 1) {
                        Text(account.name).font(.rcBodyEmph).foregroundStyle(.rcTextPrimary)
                        Text(account.email).font(.system(size: 10.5)).foregroundStyle(.rcTextTertiary)
                    }
                    Spacer(minLength: 0)
                    VStack(alignment: .trailing, spacing: 1) {
                        HStack(spacing: 3) {
                            Text("\(Double(account.xp) / 1000, specifier: "%.1f")k").font(.rcMonoSmall).foregroundStyle(.rcAccent)
                            Text("XP").font(.system(size: 10.5)).foregroundStyle(.rcTextTertiary)
                        }
                        HStack(spacing: 3) {
                            Text("\(account.replicants)").font(.rcMonoSmall).foregroundStyle(.rcAccent)
                            Text("repl").font(.system(size: 10.5)).foregroundStyle(.rcTextTertiary)
                        }
                    }
                    Image(systemName: "chevron.right").font(.system(size: 11, weight: .semibold)).foregroundStyle(.rcTextTertiary)
                }
            }
            .padding(.horizontal, Space.l).padding(.vertical, 11)
            .frame(maxWidth: .infinity, alignment: .leading)
            .overlay(alignment: .top) { Rectangle().fill(.rcSeparator).frame(height: 1) }
        }.buttonStyle(.plain)
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text("Devices").font(.system(size: 19, weight: .bold)).foregroundStyle(.rcTextPrimary)
                Text("\(Sample.devices.count)").font(.rcMonoSmall).foregroundStyle(.rcTextTertiary)
                Spacer()
                HStack(spacing: 5) {
                    Circle().fill(.rcStatusReady).frame(width: 6, height: 6)
                    Text("8 deployed").font(.rcCaption).foregroundStyle(.rcTextSecondary)
                }
            }
    ...
    var body: some View {
        HStack(spacing: 11) {
            Image(systemName: deviceSymbol(device.type)).font(.system(size: 18))
                ...
                .background(selected ? AnyShapeStyle(.rcAccentMuted) : AnyShapeStyle(.rcSurfaceRaisedStrong), in: RoundedRectangle(cornerRadius: 10))
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 8) {
                    Text(device.typeName).font(.rcBodyEmph).foregroundStyle(.rcTextPrimary)
                    Text(device.id).font(.rcMonoSmall).foregroundStyle(.rcTextTertiary)
                }
                HStack(spacing: 7) {
                    Circle().fill(DeviceStatus.tone(for: device.statusRaw).color).frame(width: 5, height: 5)
                    Text(DeviceStatus.label(for: device.statusRaw)).font(.rcCaption).foregroundStyle(.rcTextSecondary)
                    Spacer(minLength: 0)
                    Text(device.location).font(.rcMonoSmall).foregroundStyle(.rcTextTertiary)
                }
            }
            VStack(alignment: .trailing, spacing: 5) {
                Text("\(device.capacity)%").font(.rcMonoSmall).foregroundStyle(.rcTextSecondary)
                Capsule().fill(.rcSeparator).frame(width: 48, height: 4)
                    .overlay(alignment: .leading) {
        ...
        .background(selected ? AnyShapeStyle(.rcAccentMuted) : AnyShapeStyle(.clear))
        .overlay(alignment: .leading) { if selected { Capsule().fill(.rcAccent).frame(width: 3, height: 22).offset(x: 0) } }
        .overlay(alignment: .bottom) { Rectangle().fill(.rcSeparatorSoft).frame(height: 1) }
    }
}
...
struct InspectorView: View {
    ...
    private var header: some View {
        HStack(alignment: .top, spacing: 14) {
            Image(systemName: deviceSymbol(device.type)).font(.system(size: 26)).foregroundStyle(.rcAccent)
                .frame(width: 54, height: 54)
                .background(.rcAccentMuted, in: RoundedRectangle(cornerRadius: 14))
                .overlay(RoundedRectangle(cornerRadius: 14).strokeBorder(.rcAccentBorder, lineWidth: 0.5))
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 10) {
                    Text(device.typeName).font(.rcTitle).foregroundStyle(.rcTextPrimary)
                    StatusBadge(device.statusRaw, parameter: device.statusParam)
                }
                HStack(spacing: 8) {
                    Text(device.id).font(.rcMono).foregroundStyle(.rcTextSecondary)
                    Circle().fill(.rcTextTertiary).frame(width: 3, height: 3)
                    Text(device.location).font(.rcMono).foregroundStyle(.rcTextSecondary)
                }
            }
            Spacer()
        }
    ...
    private var readouts: some View {
        HStack(alignment: .top, spacing: 16) {
            VStack(spacing: 10) {
                ...
                Text("Op. Capacity").font(.system(size: 10.5, weight: .bold)).kerning(0.8).textCase(.uppercase).foregroundStyle(.rcTextTertiary)
            }
            .frame(width: 168).padding(16)
            .background(.rcSurfaceRaised, in: RoundedRectangle(cornerRadius: 14))
            .overlay(RoundedRectangle(cornerRadius: 14).strokeBorder(.rcSeparator, lineWidth: 0.5))

            VStack(alignment: .leading, spacing: 10) {
                Text("Active Task").font(.system(size: 10.5, weight: .bold)).kerning(0.8).textCase(.uppercase).foregroundStyle(.rcTextTertiary)
                if let label = device.taskLabel, let p = device.taskProgress {
                    Text(label).font(.rcHeadline).foregroundStyle(.rcTextPrimary)
                    ProgressView(value: p).tint(DeviceStatus.tone(for: device.statusRaw).color)
                    Text("\(Int(p * 100))% complete").font(.rcMonoSmall).foregroundStyle(.rcTextSecondary)
                } else {
                    Text("No active task — awaiting command").font(.rcBody).foregroundStyle(.rcTextTertiary)
                }
                Spacer(minLength: 0)
            }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 6) {
                ForEach(device.features, id: \.self) { f in
                    Text(f).font(.rcMonoSmall).foregroundStyle(.rcTextSecondary)
                        .padding(.horizontal, 9).padding(.vertical, 3)
                        .background(.rcSurfaceRaisedStrong, in: RoundedRectangle(cornerRadius: 6))
                }
            }
        }
    ...
    private func detail(_ k: String, _ v: String) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            Text(k).font(.system(size: 10, weight: .semibold)).kerning(0.6).textCase(.uppercase).foregroundStyle(.rcTextTertiary)
            Text(v).font(.rcBody).foregroundStyle(.rcTextPrimary)
        }
    }
...
struct CapacityRing: View {
    ...
    var body: some View {
        ZStack {
            Circle().stroke(.rcSeparator, lineWidth: 9)
            Circle().trim(from: 0, to: Double(pct) / 100)
                .stroke(.rcAccent, style: StrokeStyle(lineWidth: 9, lineCap: .round))
                .rotationEffect(.degrees(-90))
                .shadow(color: .rcAccent.opacity(0.4), radius: 4)
            HStack(alignment: .firstTextBaseline, spacing: 1) {
                Text("\(pct)").font(.system(size: 24, weight: .bold, design: .monospaced)).foregroundStyle(.rcTextPrimary)
                Text("%").font(.system(size: 13)).foregroundStyle(.rcTextSecondary)
            }
        }
    ...
    var body: some View {
        Button(action: action) {
            Text(title).font(.rcBodyEmph)
                ...
                .background(active ? AnyShapeStyle(.rcAccent) : AnyShapeStyle(.rcSurfaceRaisedStrong), in: RoundedRectangle(cornerRadius: 9))
                .overlay(RoundedRectangle(cornerRadius: 9).strokeBorder(.rcSeparator, lineWidth: active ? 0 : 0.5))
        }
...
struct CommandParamPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 8) {
                Text(RD_CMD[command] ?? command).font(.system(size: 11, weight: .bold)).kerning(0.5).textCase(.uppercase).foregroundStyle(.rcAccent)
                Text("·").foregroundStyle(.rcTextTertiary)
                Text(paramTitle).font(.rcBodyEmph).foregroundStyle(.rcTextPrimary)
                Spacer()
                Button { onClose() } label: { Image(systemName: "xmark").font(.system(size: 11, weight: .semibold)).foregroundStyle(.rcTextSecondary) }.buttonStyle(.plain)
            }
            ...
            case "start_mining":
                Picker("Resource", selection: $resource) { ForEach(resources, id: \.self) { Text($0) } }
                    .pickerStyle(.segmented).labelsHidden()
            default:
                Text("Confirm \(RD_CMD[command] ?? command)?").font(.rcBody).foregroundStyle(.rcTextSecondary)
            }

            HStack(spacing: 9) {
                Spacer()
                Button("Cancel") { onClose() }.buttonStyle(.plain).foregroundStyle(.rcTextSecondary)
                Button(confirmTitle) { onClose() }
                    .buttonStyle(.borderedProminent).tint(.rcAccent)
            }
        }
        .padding(16)
        .background(.rcSurfaceRaised, in: RoundedRectangle(cornerRadius: 12))
        .overlay(RoundedRectangle(cornerRadius: 12).strokeBorder(.rcAccentBorder, lineWidth: 0.5))
    }
...
struct SectionLabel: View {
    ...
    var body: some View {
        Text(text).font(.rcSectionLabel).kerning(1.2).textCase(.uppercase).foregroundStyle(.rcTextTertiary)
    }
}
...
struct PlaceholderPane: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            Image(systemName: "circle.dotted").font(.system(size: 30)).foregroundStyle(.rcTextTertiary)
            Text(title).font(.rcHeadline).foregroundStyle(.rcTextSecondary)
            Text("Not built in this scaffold yet.").font(.rcBody).foregroundStyle(.rcTextTertiary)
        }
