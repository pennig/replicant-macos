...
### 3.5 Controls (`ReplicantControls.swift`)

**Buttons** — one `ButtonStyle` per variant: `RCButtonStyle(.primary | .secondary | .destructive | .destructiveProminent | .text)`. Pass `fullWidth: true` for form CTAs. Visual reference: `Controls.html`.
