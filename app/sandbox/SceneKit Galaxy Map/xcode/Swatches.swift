...
private let tokenGroups: [TokenGroup] = [
    ...
    .init(title: "Separators", tokens: [
        ...
        .init(name: "SeparatorStrong", color: .rcSeparatorStrong, alpha: true),
    ...
    var body: some View {
        HStack(spacing: 12) {
            RoundedRectangle(cornerRadius: 9, style: .continuous)
                .fill(token.alpha ? AnyShapeStyle(.rcSurfaceRaised) : AnyShapeStyle(.clear))
                .overlay(RoundedRectangle(cornerRadius: 9, style: .continuous).fill(token.color))
                .overlay(RoundedRectangle(cornerRadius: 9, style: .continuous).strokeBorder(.rcSeparator, lineWidth: 1))
                .frame(width: 40, height: 40)
            VStack(alignment: .leading, spacing: 2) {
                Text(token.name).font(.rcBodyEmph).foregroundStyle(.rcTextPrimary)
                ...
                ForEach(tokenGroups) { group in
                    VStack(alignment: .leading, spacing: 6) {
                        Text(group.title).font(.rcSectionLabel).textCase(.uppercase).kerning(1)
                            .foregroundStyle(.rcTextTertiary).opacity(0.8)
